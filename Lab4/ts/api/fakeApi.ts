...
export class FakeApi implements IFakeApi {
    ...
    private mockProducts: ApiProduct[] = [
        {
            id: 'e1',
            name: 'iPhone 15 Pro',
            price: 129990,
    ...
    async getProductById(id: string): Promise<ApiResponse<ApiProduct>> {
        await this.simulateDelay();
    ...
    async getProductsByCategory(category: ProductCategory): Promise<ApiResponse<ApiProduct[]>> {
        await this.simulateDelay();
    ...
    async searchProducts(query: string): Promise<ApiResponse<ApiProduct[]>> {
        await this.simulateDelay();
    ...
    async createProduct(productData: Partial<ApiProduct>): Promise<ApiResponse<ApiProduct>> {
        await this.simulateDelay();
    ...
    async getStats(): Promise<ApiResponse<StoreStats>> {
        await this.simulateDelay();
        ...
        const stats: StoreStats = {
            ...
            averagePrice: Math.round(
                this.mockProducts.reduce((sum, p) => sum + p.price, 0) / this.mockProducts.length
            ),
